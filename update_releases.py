...
from utils import get_data_from_deployment_plan_file, release_updated_successfuly
...
try:
    for deployment_to_update in releases_to_update:
        ...
        if not updated_successfully:
            logging.error(f'Message:Release Update Unsuccessful - Project:{deployment_to_update.release_project_name} Release:{deployment_to_update.release_name} Destination Environment:{release_stage_name} At:{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
        else:
