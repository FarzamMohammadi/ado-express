<script>
  ...
  const checkInputValidity = () => (invalid = required && !bindValue);
  ...
  <div class="password-input mb-4">
    ...
    <div class="relative divide-x-0 divide-gray-600 dark:divide-gray-200 hover:divide-x-2 divide-y-0">
      ...
      {:else}
        ...
        />
      {/if}
      <button type="button" class="absolute inset-y-0 right-0 items-center px-2" on:click={() => (showPAT = !showPAT)}>
        <svg
          ...
          class={`feather ${showPAT ? 'feather-eye' : 'feather-eye-slash'}`}
        >
          <path
            d={showPAT
              ? 'M23.928 11.036c-.182-.28-4.441-6.936-11.928-6.936s-11.746 6.656-11.928 6.936a.5.5 0 000 .928C1.184 12.964 5.439 19.5 12 19.5s10.816-6.536 11.928-7.536a.5.5 0 000-.928zM12 16.5a4.5 4.5 0 110-9 4.5 4.5 0 010 9z'
              : 'M23.928 11.036c-.182-.28-4.441-6.936-11.928-6.936s-11.746 6.656-11.928 6.936a.5.5 0 000 .928C1.184 12.964 5.439 19.5 12 19.5s10.816-6.536 11.928-7.536a.5.5 0 000-.928zM2.5 2.5l19 19M2.5 21.5l19-19'}
          />
        </svg>
      </button>
    </div>
